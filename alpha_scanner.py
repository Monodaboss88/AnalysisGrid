...
def _build_verdict(c: Dict) -> str:
    ...
    # Convergence layer (added by Step 8 if available)
    conv = c.get("convergence", {})
    conv_score = conv.get("score", 0)
    if conv_score > 0:
        alert = conv.get("alert_type", "MIXED")
        agreement = conv.get("agreement_pct", 0)
        emoji = "🟢" if alert == "CONVERGENCE" else "🟡" if alert == "MIXED" else "🔴"
        parts.append(f"{emoji} Convergence: {conv_score:.0f}/100 ({alert}) — {agreement:.0f}% agreement")
        if alert == "DIVERGENCE":
            bears = conv.get("bears", [])
            if bears:
                parts.append(f"  ⚠️ Dissenters: {', '.join(bears)}")
...
async def run_alpha_scan(universe: str = "all", max_results: int = 5) -> Dict:
    ...
    # ── Step 8: Convergence Scoring ──
    # Measures how aligned all scanner signals are for each pick.
    # Uses enrichment data already collected in Steps 3-6, fetches the
    # 3 missing scanners (options_flow, buffett, sustainability) in parallel.
    logger.info(f"Alpha Scanner: Step 8 — Convergence Scoring ({len(results)} symbols)")
    try:
        from convergence_scanner import score_from_alpha_candidate

        async def _fetch_extra(sym: str) -> dict:
            """Fetch the 3 scanners not in the alpha pipeline."""
            extra = {}
            try:
        ...
        # Fetch extras for all results in parallel
        extras = await asyncio.gather(
            *[_fetch_extra(c["symbol"]) for c in results],
            return_exceptions=True,
        )

        # Determine profile from duration tier of top pick
        top_tier = results[0].get("duration_tier", "SWING") if results else "SWING"
        conv_profile = "daytrade" if top_tier == "DAY" else "swing" if top_tier == "SWING" else "equal"

        for c, ext in zip(results, extras):
            try:
                extra_data = ext if isinstance(ext, dict) else {}
                conv = score_from_alpha_candidate(c, extra_data=extra_data, profile=conv_profile)
                c["convergence"] = {
                    "score": conv.convergence_score,
                    "direction_score": conv.direction_score,
                    "direction_label": conv.direction_label,
                    "alert_type": conv.alert_type,
                    "conviction": conv.conviction,
                    "agreement_pct": conv.agreement_pct,
                    "bulls": conv.bulls,
                    "bears": conv.bears,
                    "neutrals": conv.neutrals,
                    "votes": conv.votes,
                    "summary": conv.summary,
                }
            except Exception as e:
                logger.debug(f"Convergence failed for {c['symbol']}: {e}")
                c["convergence"] = {"score": 0, "alert_type": "ERROR", "summary": str(e)}

        pipeline["steps"]["convergence"] = {
            "profile": conv_profile,
            "scored": sum(1 for c in results if c.get("convergence", {}).get("score", 0) > 0),
            "high_convergence": sum(1 for c in results if c.get("convergence", {}).get("score", 0) >= 70),
            "divergences": sum(1 for c in results if c.get("convergence", {}).get("alert_type") == "DIVERGENCE"),
        }
    except ImportError:
        logger.warning("Convergence scanner not available — skipping Step 8")
        for c in results:
            c["convergence"] = {"score": 0, "alert_type": "UNAVAILABLE", "summary": "Module not found"}
    except Exception as e:
        logger.error(f"Step 8 convergence scoring failed: {e}")
        for c in results:
            c["convergence"] = {"score": 0, "alert_type": "ERROR", "summary": str(e)}
    ...
    pipeline["meta"] = {
        ...
        "top_convergence": results[0].get("convergence", {}).get("score", 0) if results else 0,
