...
async def _fetch_all_scanners(ticker: str) -> dict:
    ...
    async def _fetch_regime(sym: str) -> dict:
        """Regime scanner — bull/bear/choppy day classification."""
        try:
            from regime_scanner import RegimeScanner
            rs = RegimeScanner()
            result = await asyncio.to_thread(rs.scan, sym.upper(), 30)
            if not result or result.days_analyzed == 0:
                return {}
            return result.to_dict()
        except Exception as e:
            logger.debug(f"Convergence regime scan failed for {sym}: {e}")
            return {}
